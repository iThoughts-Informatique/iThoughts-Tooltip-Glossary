...
if ( ! class_exists( __NAMESPACE__ . '\\Glossary' ) ) {
	...
	class Glossary extends Tip {
		...
		private function get_glossary_term_element_array( $term, $text = null, $datas = array() ) {
			...
			if ( function_exists( 'icl_object_id' ) ) { // If WPML is enabled &...
				if ( ! (isset( $datas['handled']['disable_auto_translation'] ) && $datas['handled']['disable_auto_translation']) ) { // ... we want auto translate...
					...
					return $this->generate_glossary( $term['ID'], $text, $datas );
				}
			}
				...
				switch ( $termcontent ) {
					case 'full':{
						if ( isset( $term['post_content'] ) && $term['post_content'] ) {
							...
							return generate_glossary( $term['ID'], $text, $datas );
						}
					}break;
		...
		public function generate_glossary( $term, $text = null, $options = array() ) {
			if ( $term instanceof \ithoughts\v1_0\PseudoPost ) {
				$term = $term->to_WP_Post();
			}
			$backbone = \ithoughts\tooltip_glossary\Backbone::get_instance();
			$datas = apply_filters( 'ithoughts_tt_gl-split-args', $options );
			$opts = array_replace_recursive(array(), $backbone->get_serverside_options(), $datas['options']);
			$datas['options'] = &$opts;
			...
			if ( $backbone->get_option( 'staticterms' ) ) {
				...
				if ( isset( $datas['attributes']['termcontent'] ) ) {
					...
					$termcontent = isset( $opts['termcontent'] ) ? $opts['termcontent'] : $backbone->get_option('termcontent');
				}
				...
				echo 'Transmit termcontent';
				if ( isset ($opts['termcontent']) ) {
					$datas['attributes']['data-termcontent'] = $opts['termcontent'];
				}
				if ( $term instanceof \WP_Post ) {
					$datas['attributes']['data-termid'] = $term->ID;
					if ( is_null( $text ) ) {
			...
			if ( ! isset( $datas['linkAttrs']['href'] ) ) {
				$href = 'javascript::void(0)';
				if ( $opts['termlinkopt'] != 'none' ) { // If there need a link
					if ( null !== $term ) {
						$href = apply_filters( 'ithoughts_tt_gl_term_link', \ithoughts\v6_0\Toolbox::get_permalink_light( $term,'glossary' ) );
					} else {
		...
		public function glossary_shortcode( $atts, $text = '' ) {
			$id = isset( $atts['glossary-id'] ) && intval($atts['glossary-id']) > 0 ? intval($atts['glossary-id']) : null;
			return apply_filters( 'generate_glossary', $id, $text, $atts );
		}
