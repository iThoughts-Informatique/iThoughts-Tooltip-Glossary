<form id="customizing-form">
	<table class="form-table stripped0">
		<thead>
			<tr>
				<th>
					<label for="theme_name"><?php _e('Theme name', 'ithoughts_tooltip_glossary'); ?></label>
				</th>
				<td colspan="3">
					<input type="text" autocomplete="off" name="theme_name" id="theme_name" required minlength="3" pattern="^[a-zA-Z0-9][a-zA-Z0-9\-\_]+[a-zA-Z0-9]" data-pattern-infos="<?php _e('At least 3 letters (lowercase and uppercase), numbers, _ or -, and not starting or ending with symbol', 'ithoughts_tooltip_glossary'); ?>">
				</td>
			</tr>
			<tr>
				<td></td>
...
foreach($prefixs as $prefix){
				?>
				<td>
					<!--<label for="<?php echo $prefix; ?>_bt" class="block"><?php _e('Type', 'ithoughts_tooltip_glossary'); ?>&nbsp;
					...
					<!--<div data-switcher-mode="gradient">
						<select name="<?php echo $prefix; ?>_grad_dir" value="t_b" id="<?php echo $prefix; ?>_grad_dir">
							<option value="t_b"><?php _e('Top to bottom', 'ithoughts_tooltip_glossary'); ?></option>
							<option value="b_t"><?php _e('Bottom to top', 'ithoughts_tooltip_glossary'); ?></option>
							<option value="l_r"><?php _e('Left to right', 'ithoughts_tooltip_glossary'); ?></option>
							<option value="r_l"><?php _e('Right to left', 'ithoughts_tooltip_glossary'); ?></option>
						</select>
...
foreach($prefixs as $prefix){
					...
					<table style="margin:0px auto;width:100%">
						<tr>
							...
							<td style="padding:0;" rowspan="2">
								...
								<label for="sh-i">
									<?php _e("Inset", "ithoughts_tooltip_glossary"); ?>:&nbsp;
						...
						<tr>
							...
							<td style="padding:0;">
								<label for="sh-h">
									<?php _e("Height", "ithoughts_tooltip_glossary"); ?><br/>
			...
			<tr>
				...
				<td>
					<label for="border-w">
						<?php _e("Border width", "ithoughts_tooltip_glossary"); ?>:&nbsp;
						<input autocomplete="off" type="text" class="input-width" name="border-w" id="border-w" value="1px" pattern="^(0|\d+(r?em|px|%|ex|pt|(c|m)m|in|pc|v(h|w|min|max)))$" data-pattern-infos="<?php _e('Valid border-width value: 0, inherit, initial, or a value in rem, em, px, %, ex, pt, cm, mm, in, pc, vh, vw, vmin, or vmax. Negatives not accepted', 'ithoughts_tooltip_glossary'); ?>"/>
					</label>
				</td>
				<td>
					<label for="border-s">
						<?php _e("Border style", "ithoughts_tooltip_glossary"); ?>:&nbsp;
						<select name="border-s" id="border-s" value="solid">
							<option value="solid"><?php _e("Solid", "ithoughts_tooltip_glossary"); ?></option>
							<option value="dotted"><?php _e("Dotted", "ithoughts_tooltip_glossary"); ?></option>
							<option value="dashed"><?php _e("Dashed", "ithoughts_tooltip_glossary"); ?></option>
							<option value="double"><?php _e("Double", "ithoughts_tooltip_glossary"); ?></option>
							<option value="groove"><?php _e("Groove", "ithoughts_tooltip_glossary"); ?></option>
							<option value="ridge"><?php _e("Ridge", "ithoughts_tooltip_glossary"); ?></option>
							<option value="inset"><?php _ex("Inset", "Border property", "ithoughts_tooltip_glossary"); ?></option>
							<option value="outset"><?php _e("Outset", "ithoughts_tooltip_glossary"); ?></option>
							<option value="none"><?php _e("None", "ithoughts_tooltip_glossary"); ?></option>
							<option value="hidden"><?php _e("Hidden", "ithoughts_tooltip_glossary"); ?></option>
						</select>
					</label>
				</td>
				<td>
					<label for="border-c">
						<?php _e("Border color", "ithoughts_tooltip_glossary"); ?>:&nbsp;
...
foreach($prefixs as $prefix){
			...
			<tr>
				<th><?php _e('Line height', 'ithoughts_tooltip_glossary'); ?></th>
				<?php
foreach($prefixs as $prefix){
				?>
...
foreach($prefixs as $prefix){
			...
			<tr>
				<th><?php _e('Text emphasis color', 'ithoughts_tooltip_glossary'); ?></th>
				<td colspan="2"></td>
...
foreach($prefixs as $prefix){
				...
				<td>
					<select name="<?php echo $prefix; ?>_tf" required>
						<option value=""><?php _e('Please wait...', 'ithoughts_tooltip_glossary'); ?></option>
					</select>
				</td>
				<?php
}
			...
			<tr>
				<th><?php _e('Other custom CSS', 'ithoughts_tooltip_glossary'); ?> <span class="ithoughts_tooltip_glossary-tooltip" data-tooltip-nosolo="true" data-tooltip-content="<?php echo rawurlencode(__('Following CSS rules will be copied to output compiled CSS, into the appropriated CSS rules. Don\'t specify the selector.', 'ithoughts_tooltip_glossary')); ?>"><a href="javascript:void(0)">(<?php _e('infos', 'ithoughts_tooltip_glossary'); ?>)</a></span></th>
				<?php
foreach($prefixs as $prefix){
				?>
				<td>
			...
			<tr>
				<th><?php _e('Other CSS rules', 'ithoughts_tooltip_glossary'); ?> <span class="ithoughts_tooltip_glossary-tooltip" data-tooltip-nosolo="true" data-tooltip-content="<?php echo rawurlencode(__('Will be copied at the end of the CSS', 'ithoughts_tooltip_glossary')); ?>"><a href="javascript:void(0)">(<?php _e('infos', 'ithoughts_tooltip_glossary'); ?>)</a></span></th>
				<td colspan="3">
			...
			<tr>
				<td colspan="4">
					<input autocomplete="off" type="submit" name="compilecss" id="compilecss" class="alignleft button-primary" value="<?php _e("Compile CSS", 'ithoughts_tooltip_glossary'); ?>" style="width:100%;text-align:center;">
				</td>
			</tr>
			<tr>
				<th>
					<?php _e("Output CSS", 'ithoughts_tooltip_glossary'); ?><br />
					(<?php _e("Copy the following CSS rules to your <a href=\"https://codex.wordpress.org/Child_Themes\" target=\"_blank\">child theme</a>", 'ithoughts_tooltip_glossary'); ?>)
				</th>
				<td colspan="3">
					<textarea name="outputcss" id="outputcss" style="width: 100%;height:250px;" readonly="readonly">
					</textarea>
					<button id="copyclip" class="alignleft button-secondary" style="width:100%;text-align:center;" disabled="disabled"><?php _e("Copy to clipboard", 'ithoughts_tooltip_glossary'); ?></button>
					<div id="copyclipres" data-text-error="<?php _e("Could not copy to clipboard", 'ithoughts_tooltip_glossary'); ?>" data-text-success="<?php _e("Successfully copied to clipboard", 'ithoughts_tooltip_glossary'); ?>"></div>
				</td>
			</tr>
		</tbody>
	</table>
