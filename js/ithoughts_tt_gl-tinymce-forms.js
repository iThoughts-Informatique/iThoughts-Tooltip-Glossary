	...
	$d.ready(function(){
		...
		(function initAdvancedConfiguration(){
			...
			$opts.find('input[type="checkbox"].ithoughts-tristate').attr("data-state", function(i, attrVal){
				...
				console.log(this.checked, this.indeterminate);
			}).change();
			...
			$("#qtip-keep-open").change(function(){
				...
				return function lockQtipTrigger(){
					if(this.checked){
						...
						modeInput.get(0).disabled = !(textInput.get(0).disabled = false);
						modeInput.val(textInput.val() = "click");
						modeInput.val(textInput.val() = oldValue);
					} else {
