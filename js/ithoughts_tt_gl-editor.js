...
/*global QTags: false, ithoughts_tt_gl_editor: false */
	...
	ithoughts.$d.ready( function(){
		...
		i_t_g_e.removeAccents = function( s ) {
			var translate_re	= /[¹²³áàâãäåaaaÀÁÂÃÄÅAAAÆccç©CCÇÐÐèéê?ëeeeeeÈÊË?EEEEE€gGiìíîïìiiiÌÍÎÏ?ÌIIIlLnnñNNÑòóôõöoooøÒÓÔÕÖOOOØŒr®Ršs?ßŠS?ùúûüuuuuÙÚÛÜUUUUýÿÝŸžzzŽZZ]/g,
				 translate		= {
					'¹': '1', '²': '2', '³': '3', 'á': 'a', 'à': 'a', 'â': 'a', 'ã': 'a', 'ä': 'a', 'å': 'a', a:   'a', 'À': 'a', 'Á': 'a', 'Â': 'a', 'Ã': 'a', 'Ä': 'a', 'Å': 'a', A:   'a', 'Æ': 'a', c:   'c', 'ç': 'c', '©': 'c', C:   'c', 'Ç': 'c', 'Ð': 'd', 'è': 'e', 'é': 'e', 'ê': 'e', '?': 's', 'ë': 'e', e:   'e', 'È': 'e', 'Ê': 'e', 'Ë': 'e', E:   'e', '€': 'e', g:   'g', G:   'g', i:   'i', 'ì': 'i', 'í': 'i', 'î': 'i', 'ï': 'i', 'Ì': 'i', 'Í': 'i', 'Î': 'i', 'Ï': 'i', I:   'i', l:   'l', L:   'l', n:   'n', 'ñ': 'n', N:   'n', 'Ñ': 'n', 'ò': 'o', 'ó': 'o', 'ô': 'o', 'õ': 'o', 'ö': 'o', o:   'o', 'ø': 'o', 'Ò': 'o', 'Ó': 'o', 'Ô': 'o', 'Õ': 'o', 'Ö': 'o', O:   'o', 'Ø': 'o', 'Œ': 'o', r:   'r', '®': 'r', R:   'r', 'š': 's', s:   's', 'ß': 's', 'Š': 's', S:   's', 'ù': 'u', 'ú': 'u', 'û': 'u', 'ü': 'u', u:   'u', 'Ù': 'u', 'Ú': 'u', 'Û': 'u', 'Ü': 'u', U:   'u', 'ý': 'y', 'ÿ': 'y', 'Ý': 'y', 'Ÿ': 'y', 'ž': 'z', z:   'z', 'Ž': 'z', Z:   'z',
				};
				...
				return content.replace( /\[(?:itg-)?(glossary|tooltip|mediatip)(?!_)(.*?)\](.*?)\[\/(?:itg-)?(glossary|tooltip|mediatip)\]/g, function( all, balise, inner, text ) {
					var attrs	= {},
						 ...
						 ret		= '<a data-type="' + prefix3 + '-' + {
							glossary: 'term', tooltip:  'tooltip', mediatip: 'mediatip',
						}[balise] + '"',
						 i;
			...
			function( content ) { // For [glossary]
				return content.replace( /<a\s+(?=[^>]*data-type="ithoughts-tooltip-glossary-(term|tooltip|mediatip)")(.*?)>(.*?)<\/a>/g, function( all, type, inner, text ) {
					var attrs	= {},
						 ...
						 matched	= null,
						 b		= {
							term:     'glossary', tooltip:  'tooltip', mediatip: 'mediatip',
						}[type],
						 ret		= '[' + prefix4 + '-' + b,
						 i;
					while ( Boolean( matched = regex.exec( inner )) === true ) {
		...
		i_t_g_e.editorForms = {
			list: function glossarylistfct( selection, callback ) {
