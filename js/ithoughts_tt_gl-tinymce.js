...
(function(ithoughts) {
    ...
    tinymce.PluginManager.add('ithoughts_tt_gl_tinymce', function registerTinyMCEPlugin(editor, url) {
            ...
            $.ajax({
                ...
                success: function createTinyMCEFormAjaxed(out){
                    var newDom = $($.parseHTML(out, true)),
                        h = 400,
                        w = 455,
                        popupTooltip = newDom.find("#ithoughts_tt_gl-tooltip-form"),
                        popupTooltipOptions = newDom.find("#ithoughts_tt_gl-tooltip-form-options");
                    ...
                    newDom.css({opacity: 0}).animate({opacity:1}, 500);
                    console.log(document.body,$(document.body),newDom);
                            ...
                            switch(data.type){
                                case "glossary": {
                                    if(data.term_id == "" || data.text == "")
                                        ...
                                        editor.insertContent('[ithoughts_tooltip_glossary-glossary glossary-id="'+data.glossary_id+'"' + (data.disable_auto_translation ? " disable_auto_translation=\"true\"" : "" ) + (data.link && data.link.length > 0 ? ' href="' + encodeURI(data.link) + '"' : "") + ']'+data.text+"[/ithoughts_tooltip_glossary-glossary]" + ((mode != "load") ? " " : ""));
                                } break;
