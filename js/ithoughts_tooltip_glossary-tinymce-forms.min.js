...
o.removeClass("hidden"),setTimeout(e,25),o.find(".option").on("focusout",i).on("click",function(t){$('[name="glossary_term_id"]').val(t.currentTarget.getAttribute("data-id")),n[0].value=$(t.currentTarget).find("p > b").text(),o.addClass("hidden")})}$(".tabs li").click(function(t){if(!$(this).hasClass("active")){$(this).parent().find(".active").removeClass("active"),$(this).addClass("active"),$(t.target).parent().parent().find(" > .active").removeClass("active")
var e=$(t.target).index()
switch(console.log(e),$($(t.target).parent().parent().children()[e+1]).addClass("active"),linkInput=$("#itghouts_tt_gl_link"),e){case 0:linkInput.disabled=!0
break
...
var n=$("#glossary_term"),o=$("#glossary_term_completer")
$w.resize(e)
var r=null
n.on("keyup focusin",function(){r&&r.abort(),searchedString=removeAccents($(this).val().toLowerCase()),r=$.ajax({url:ithoughts_tt_gl_tinymce_form.admin_ajax,method:"POST",dataType:"json",data:{action:"ithoughts_tt_gl_get_terms_list",search:searchedString},complete:function(t){var e=t.responseJSON
...
var l=["span","link"]
l.forEach(function(t){console.log(t),$("#kv-pair-"+t+"-attrs-add").click(function(){var e=$("#attributes-"+t+"-key"),i=$("#attributes-"+t+"-value"),a=$("#kv-pair-"+t+"-attrs")
